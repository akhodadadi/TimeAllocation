#In the paper this model is parameterized slightly differently.
#The parameters in the paper are obtained from the parameters
#here as follows:

#\hat{rho}=alpha_c
#alpha_{i,phi}=((alpha_mi*ci)/(alpha_c))
#alpha_{i,c}=ci
#also scale he rewards:
#ri=alpha_c*ri/ci

NLL_Model9 = function(x,parameter)
{
  #===parameters===
  #---free parameters---
  ...
  if (whatToDo==1){#fitting
    ...
    output = list(b1Vect=bMat[1,],b2Vect=bMat[2,],vMat=vMat)
