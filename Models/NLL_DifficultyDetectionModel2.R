NLL_DifficultyDetectionModel2 = function(x,parameter){
  #===parameters===
  #---free parameters---
  sigma1=x[1];sigma2=x[2];
  m10=x[3];m20=x[4];
  alpha_c=x[5];alpha_m1=x[6];alpha_m2=x[7];
  rhoWin = floor(x[8]);#window size for estimating rho
  V10=x[9];V20=x[10];V30=x[11];
  t_detect_max=x[12];th_D=x[13];
  #---free parameters---
  
  #---other---
  conditionVect=parameter$conditionVect;
  conditionVect[conditionVect==0]=3;#bonus trials are state 3
  rewardVect=parameter$rewardVect;
  positionVect=parameter$positionVect;
  canoePathMat = parameter$canoePathMat
  ...
  vMat = matrix(0,nrow=3,ncol=numOfTrials);
  vMat[1,1]=V10;vMat[2,1]=V20;vMat[3,1]=V30;
  
  #---determine detected states in all trials---
  detectedState=rep(2,numOfTrials)
  
  for (k in 1:numOfTrials){
    if (conditionVect[k]==3){#bonus trial
      detectedState[k]=3;next;
    }
  ...
  for (k in 1:(numOfTrials-1)){
    ...
    #---compute delta---
    delta = rewardVect[k] - rho[k]*rtVect[k] +
  ...
  if (whatToDo==1){#fitting
    ...
    output = list(m1Vect=mMat[1,],m2Vect=mMat[2,],
                  vMat=vMat,detectedState=detectedState,
                  rho_hat=rho)
  }
  #===compute the likelihood===
}
